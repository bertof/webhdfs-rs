...
use http::{method::Method, request::Builder as RequestBuilder, uri::Scheme};
use hyper::{
    body::to_bytes,
...
async fn error_and_ct_filter(ct_required: RCT, res: Response<Body>) -> Result<Response<Body>> {
    ...
    if status.is_success() {
        if match_mimes(&ct, ct_required) {
            ...
            Err(
                app_error!(generic "Invald content type: required='{:?}' found='{:?}'", ct_required, ct),
            )
        }
    } else {
        ...
        if match_mimes(&ct, RCT::JSON) {
            match to_bytes(res.into_body()).await {
                Ok(buf) => match serde_json::from_reader::<_, RemoteExceptionResponse>(
                    buf.clone().reader(),
                ) {
...
#[inline]
pub fn data_owned(d: Vec<u8>) -> Data {
    std::borrow::Cow::Owned(d)
}

#[cfg(feature = "zero-copy-on-write")]
#[inline]
pub fn data_borrowed(d: &'static [u8]) -> Data {
    std::borrow::Cow::Borrowed(d)
}

#[inline]
pub fn data_empty() -> Data {
...
impl ErrorD {
    #[inline]
    pub fn new(error: Error, data_opt: Option<Data>) -> Self {
        Self { error, data_opt }
    }
    #[inline]
    pub fn d(error: Error, data: Data) -> Self {
        Self::new(error, Some(data))
    }
    #[inline]
    pub fn lift(error: Error) -> Self {
        Self::new(error, None)
    }
    #[inline]
    pub fn drop(Self { error, data_opt: _ }: Self) -> Error {
        error
    }
}
...
impl HttpxEndpoint {
    pub fn new(uri: Uri, https_settings: Option<HttpsSettingsPtr>) -> Self {
        Self {
            uri,
            https_settings,
        }
    ...
    pub fn https_settings(&self) -> &Option<HttpsSettingsPtr> {
        &self.https_settings
    }
}
...
impl HttpxClient {
    fn new(endpoint: &HttpxEndpoint) -> Self {
        Self {
            endpoint: Httpx::new(endpoint),
        }
    }
    ...
    fn create_request(&self, method: Method, uri: Uri) -> RequestBuilder {
        trace!("{} {}", method, uri);
        RequestBuilder::new().method(method).uri(uri)
    }
    ...
    async fn new_get_like(endpoint: HttpxEndpoint, method: Method) -> Result<Response<Body>> {
        Self::new(&endpoint)
            .get_like_future(endpoint.uri, method)
            .await
    }

    async fn new_post_like(
        endpoint: HttpxEndpoint,
        method: Method,
        payload: Data,
    ) -> Result<Response<Body>> {
        Self::new(&endpoint)
            .post_like_future(endpoint.uri, method, payload)
            .await
    }
}
...
impl HttpyClient {
    pub fn new(endpoint: HttpxEndpoint, natmap: NatMapPtr) -> Self {
        Self { endpoint, natmap }
    }
    ...
    async fn redirect_uri(
        ...
        method: Method,
        natmap: NatMapPtr,
    ) -> Result<HttpxEndpoint> {
        let https_settings = endpoint.https_settings().clone();
        let r = HttpxClient::new_get_like(endpoint, method).await?;
        trace!(
            "Redirect: Response {} location={:?}",
            r.status(),
            r.headers().get(hyper::header::LOCATION)
        );
        match redirect_filter(r) {
            Ok(b) => Err(
                app_error!(generic "Expected redirect, found non-redirect response status={}", b.status()),
            ),
            Err(e) => match e.to_http_redirect() {
                Ok((_code, location)) => match location.parse() {
                    ...
                    Err(e) => {
                        Err(app_error!((cause=e) "Cannot parse location URI returned by redirect"))
                    }
                },
                Err(e) => Err(e),
            },
    ...
    pub async fn post_binary(self, method: Method, data: Data) -> DResult<()> {
        ...
        match HttpyClient::redirect_uri(endpoint, method.clone(), natmap).await {
            Ok(endpoint) => {
                inner(endpoint, method, data)
                    .map(|fr| fr.map_err(ErrorD::lift))
                    .await
            }
