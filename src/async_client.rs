...
use crate::op::*;
pub use crate::rest_client::{DResult, Data, ErrorD};
use crate::rest_client::{HttpxEndpoint, HttpyClient};
use crate::uri_tools::*;
use bytes::Bytes;
use futures::Stream;
use http::{uri::Parts as UriParts, Method, Uri};
...
impl HdfsClientBuilder {
    ...
    pub fn from_explicit_config(conf: Config) -> Self {
        ...
        Self {
            c: HdfsClient {
                ...
                doas: conf.doas,
                dt: conf.dt,
                https_settings: conf.https_config.map(|s| https_settings_ptr(s.into())),
            },
        }
    }
    ...
    pub fn from_config() -> Self {
        Self::from_explicit_config(read_config())
    }
    ...
    pub fn from_config_opt() -> Option<Self> {
        read_config_opt().map(Self::from_explicit_config)
    }

    pub fn alt_entrypoint(self, alt_entrypoint: Uri) -> Self {
        Self {
            c: HdfsClient {
                alt_entrypoint: Some(alt_entrypoint.into_parts()),
                ..self.c
            },
        }
    }
    pub fn https_settings(self, https_settings: HttpsSettings) -> Self {
        Self {
            c: HdfsClient {
                https_settings: Some(https_settings_ptr(https_settings)),
                ..self.c
            },
        }
    }
...
impl HdfsClient {
    ...
    fn https_settings(&self) -> Option<HttpsSettingsPtr> {
        self.https_settings.clone()
    }

    fn path_and_query(&self, file_path: &str, op: Op, args: Vec<OpArg>) -> Vec<u8> {
        let q = PathEncoder::new(Self::SVC_MOUNT_POINT)
            .extend(file_path)
            .query();
        let q = if let Some(user) = &self.user_name {
            q.add_pv("user.name", user)
        } else {
            q
        };
        let q = if let Some(doas) = &self.doas {
            q.add_pv("doas", doas)
        } else {
            q
        };
        let q = if let Some(dt) = &self.dt {
            q.add_pv("delegation", dt)
        } else {
            q
        };
    ...
    fn uri(&self, fostate: FOState, pq: &[u8]) -> FOResult<Uri> {
        ...
        let ep = if fostate.is_alt() {
            if let Some(ep) = &self.alt_entrypoint {
                ep
            } else {
                &self.entrypoint
            }
        ...
        if let Some(authority) = &ep.authority {
            b = b.authority(authority.clone());
        }

        let r = b.path_and_query(pq).build().aerr_f(|| {
            format!(
                "Could not build URI: path_and_query={}",
                String::from_utf8_lossy(pq)
            )
        });
    ...
    fn httpc(&self, fostate: FOState, pq: &[u8]) -> FOResult<HttpyClient> {
        ...
        Ok((
            HttpyClient::new(HttpxEndpoint::new(uri, https_settings), natmap),
            fostate,
        ))
    }
    ...
    fn failover_fsm<T>(&self, fostate: FOState, result: Result<T>) -> (FOAction<T, ()>, FOState) {
        match result {
            Err(e) if self.alt_entrypoint.is_some() && Self::is_standby_error(&e) => {
                (FOAction::FailOver(()), fostate.next())
            }
    ...
    fn failover_fsm_d<T>(
        ...
        fostate: FOState,
        result: DResult<T>,
    ) -> (FOAction<T, Data>, FOState) {
        match result {
            Err(ErrorD {
                error,
                data_opt: Some(data),
            }) if self.alt_entrypoint.is_some() && Self::is_standby_error(&error) => {
            ...
            Ok(v) => (FOAction::Proceed(Ok(v)), fostate),
        }
    }
    ...
    {
        with_failover!(
            [|r: HttpyClient| r.get_json(), |r: HttpyClient| r.get_json()],
            self,
            fostate,
    ...
    async fn data_op<'t>(
        ...
        fn nod((error, fostate): (Error, FOState)) -> (ErrorD, FOState) {
            (
                ErrorD {
                    error,
                    data_opt: None,
