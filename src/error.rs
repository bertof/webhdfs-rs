...
impl Error {
    pub fn new(msg: Option<Cow<'static, str>>, cause: Cause) -> Self {
        Error { msg, cause }
    }
    pub fn anon(cause: Cause) -> Self {
        Self::new(None, cause)
    }
    ...
    pub fn app_c(msg: &'static str) -> Self {
        Error::new(Some(Cow::Borrowed(msg)), Cause::None)
    }
    pub fn app_s(msg: String) -> Self {
        Error::new(Some(Cow::Owned(msg)), Cause::None)
    }
    ...
    pub fn timeout_c(msg: &'static str) -> Self {
        Self::new(Some(Cow::Borrowed(msg)), Cause::Timeout)
    }
}
...
impl Display for Error {
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        ...
        match &self.cause {
            ...
            Cause::HttpInvalidUriParts(e) => {
                write!(f, "; caused by http::uri::InvalidUriParts: {}", e)
            }
            ...
            Cause::HttpRedirect(code, location) => {
                write!(f, "; caused by HTTP redirect {} {}", code, location)
            }
...
pub trait IntoErrorAnnotated: Sized {
    ...
    fn into_with_c(self, msg: &'static str) -> Error {
        self.into_with(Cow::Borrowed(msg))
    }
    fn into_with_s(self, msg: String) -> Error {
        self.into_with(Cow::Owned(msg))
    }
}

impl IntoErrorAnnotated for Error {
    fn into_with(self, msg: Cow<'static, str>) -> Error {
        self.with_msg_prepended(msg)
    }
}
...
impl<T, E> AnnotateError<T> for std::result::Result<T, E>
where
    E: IntoErrorAnnotated,
{
    fn aerr(self, msg: &'static str) -> Result<T> {
...
macro_rules! error_conversion {
    ($f:ident($t:ty)) => {
        impl From<$t> for Error {
            #[cfg(panic_on_error)]
            fn from(e: $t) -> Self {
                panic!(Error::anon(Cause::$f(e)).to_string())
            }
            #[cfg(not(panic_on_error))]
            fn from(e: $t) -> Self {
                Error::anon(Cause::$f(e))
            }
        }
...
macro_rules! error_conversion_noarg {
    ($f:ident($t:ty)) => {
        impl From<$t> for Error {
            #[cfg(panic_on_error)]
            fn from(_: $t) -> Self {
                panic!(Error::anon(Cause::$f).to_string())
            }
            #[cfg(not(panic_on_error))]
            fn from(_: $t) -> Self {
                Error::anon(Cause::$f)
    ...
    fn from(e: Error) -> Self {
        ...
        match e {
            ...
            Error {
                msg: Some(m),
                cause: Cause::Timeout,
            } => IoError::new(IoErrorKind::TimedOut, m),
            Error {
                msg: None,
                cause: Cause::Timeout,
            } => IoError::from(IoErrorKind::TimedOut),
